...
func New(log *logger.Logger, addr string, route string, readTimeout, writeTimeout time.Duration, idleTimeout time.Duration) *Expvar {
	mux := httptreemux.New()
	exp := Expvar{
		log: log,
		server: http.Server{
	...
	go func() {
		...
		log.Info(ctx, "expvar", "status", "API listening", "host", addr)
