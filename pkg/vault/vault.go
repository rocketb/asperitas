// This implements vault keystore for JWT support.
...
// SystemSealStatusResponse represents the response from a system seal status
// call.
type SystemSealStatusResponse struct {
	Sealed bool `json:"sealed"`
}

// This provides a default client configuration
...
// SystemInit provide support to initialize a vault system for use.
func (v *Vault) SystemInit(ctx context.Context, shares int, threshold int) (SystemInitResponse, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPut, url, &b)
	if err != nil {
		return SystemInitResponse{}, fmt.Errorf("creating request: %w", err)
	}
	...
	if err != nil {
		...
		return SystemInitResponse{}, fmt.Errorf("executing request: %w", err)
	}
	defer resp.Body.Close()
	...
	if err := json.NewDecoder(resp.Body).Decode(&response); err != nil {
		return SystemInitResponse{}, fmt.Errorf("json decoding: %w", err)
	}

	return response, nil
}

// Unseal does what vault unseal command does.
...
func (v *Vault) Mount(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("getting mount list: %w", err)
	}
...
// CheckSealStatus gets vault seal status.
func (v *Vault) CheckSealStatus(ctx context.Context) (SystemSealStatusResponse, error) {
	url := fmt.Sprintf("%s/v1/sys/seal-status", v.address)

	req, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)
	if err != nil {
		return SystemSealStatusResponse{}, fmt.Errorf("creating request: %w", err)
	}
	...
	resp, err := v.client.Do(req)
	if err != nil {
		return SystemSealStatusResponse{}, fmt.Errorf("executing request: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return SystemSealStatusResponse{}, fmt.Errorf("status code: %d err: %w", resp.StatusCode, err)
	}

	var response SystemSealStatusResponse
	if err := json.NewDecoder(resp.Body).Decode(&response); err != nil {
		return SystemSealStatusResponse{}, fmt.Errorf("json decoding: %w", err)
	}
...
// retrieveKID perform the HTTP call against the Vault service for the
...
func (v *Vault) listMounts(ctx context.Context) (map[string]any, error) {
...
func toPublicPEM(privateKeyPEM string) (string, error) {
	...
	if block, _ = pem.Decode([]byte(privateKeyPEM)); block == nil {
		return "", errors.New("invalid key: Key must be a PEM encoded or PKCS1 or PKCS8 key")
	}
